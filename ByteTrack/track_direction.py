...
#This file is used to track a single folder of image sequences (one video)
...
def activate_tracking(frames_folder, weight_file):
    yolov7 = YOLOv7()
    yolov7.load(weight_file, classes='data.yaml', device='cpu') # use 'gpu' for CUDA GPU inference
    #Visdrone (second challenge) sequences offered below for convenience! 
    ...
    try:
        for frame_num, frame_file in enumerate(frame_files):
            ...
            if frame_num != previous_num_frames:  # Check if num_frames is updated
                ...
                for i in range(len(cls_list)): #outputs all YOLO emulated metrics for tracking evaluation
...
def main():
    ...
    if args.source:
        activate_tracking(str(args.source), str(args.weights))
